...
{ 
    ...
    if(ack1!=0&&ack2!=0) {
        return ((bool) strstr(USART_ReceiveString, ack1) || 
                         (bool) strstr (USART_ReceiveString, ack2 ) );
    } else if( ack1 != 0 )  //strstr(s1,s2);检测s2是否为s1的一部分，是返回该位置，否则返回false，它强制转换为bool类型了
        return ((bool) strstr(USART_ReceiveString, ack1));
	else
        return ((bool) strstr(USART_ReceiveString, ack2));
...
{
    char cCmd [120];
    sprintf ( cCmd, "AT+MQTTSUB=0,\"%s\",0",Topic );
    return ESP8266_Send_AT_Cmd( cCmd, "OK", NULL, 500 );
}
